...
`allocatedAmount` can be caulculated earlier in the function and then used in the test on line 256.
...
`usrClaim.amountWithdrawn` is accessed twice from storage. It can be copied into a memory variable on the first access and then that memory variable used on line 377 to save gas.